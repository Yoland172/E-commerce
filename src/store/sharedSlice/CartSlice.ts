...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        setProductsQuantityAndId: (state, action: PayloadAction<extractedProductsList[]>) => {
    ...
    return async (dispatch, getState) => {
        try {
            ...
            if (cartfromStorage == null && id) {
                const stockProductsQuantityAndId = getState().cartState.productsQuantityAndId;
                dispatch(setIsFetching());
                const res = await putChangedQuantityProduct(id, stockProductsQuantityAndId);
                if (res) {
...
export const putChangedQuantityProductThunk = (userId: number, productId: number, quantity: number): AppThunk => {
    return async (dispatch, getState) => {
        try {
            dispatch(setIsFetching());
            const stockProductsQuantityAndId = getState().cartState.productsQuantityAndId;
            let prodToUse = stockProductsQuantityAndId;

            if (stockProductsQuantityAndId.length === 0) {
                const storageCart = getUserCartFromStorage();
                if (storageCart !== null) {
                    prodToUse = storageCart.productsQuantityAndId;
                }
            }

            console.log(prodToUse);
            const extractedArray = mergeStockArray(prodToUse, productId, quantity);
...
export const deleteProductFromCart = (userId: number, productId: number): AppThunk => {
    return async (dispatch, getState) => {
        try {
            const productsQuantityAndId = getState().cartState.productsQuantityAndId;
            const editedProductsQuantityAndId = deleteItemFromCart(productsQuantityAndId, productId);
            const res = await putChangedQuantityProduct(userId, editedProductsQuantityAndId);
            if (res) {
                dispatch(setCart(res));
...
export const { setCart, setIsFetching, deleteItem, setProductsQuantityAndId, removeAllData } = cartSlice.actions;
